...
export const getAttachments =
    ...
    async (dispatch: Dispatch): Promise<any> => {
        ...
        try {
            const attachments: any = await axioscall.post('/get-attachments', {
                projectId,
            });
...
export const getLightSelectionsForProject =
    ...
    async (dispatch: Dispatch): Promise<any> => {
        ...
        try {
            const lightSelections: any = await axioscall.post(
                '/get-lightSelections-for-project',
                { projectId }
            );

            if (lightSelections?.data?.lightSelections?.length) {
                dispatch(
                    setLightSelections(lightSelections.data.lightSelections)
                );
