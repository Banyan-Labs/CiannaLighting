...
import { getProject, setTheRoom, getAllProjectRoomsAction, deleteThisRoom, editThisRoom } from '../../../redux/actions/projectActions';
...
export const DeleteModal: FC<Props> =
 ({ closeModal, openModal, light, setDeleteLight, setDeleteRoom, deleteRoom, room, editRoom, setEditRoom }) => {
    ...
    const onSubmit1 = async () => {
        try {
           !deleteRoom ?  await dispatch(deleteLight({roomId: String(storedRoomId), _id: String(light._id)})) :
           await dispatch(deleteThisRoom({_id: String(storedRoomId), projectId: String(storedProjId)}))
            ...
            <div className="new-room-modal-container">
                <div className="modal-title-close-btn">
                    ...
                    <div className='delete-Modal-container'>
                                <h4>{deleteRoom ? `Delete ${room?.name}` : `Delete ${light?.item_ID} From ${room?.name}`}</h4>
                                    <button
                                    onClick={onSubmit1}
                                        type="submit"
                   ...
                   <div className="new-room-modal-title">
                       <h3 className="modal-title"> {`Edit ${room?.name}`}</h3>
                   </div>
